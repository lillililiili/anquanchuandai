...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.pumpAndSettle();
      final open = find.byKey(const ValueKey('events-quick-open'));
      await tester.ensureVisible(open);
      await tester.tap(open);
      await tester.pumpAndSettle();
      expect(queries.last['status'], 'open');
      expect(
      ...
      expect(find.text('未关闭 35'), findsOneWidget);
      ...
      expect(
        queries.any((q) => q['current'] == 2 && q['status'] == 'open'),
        true,
      );
      ...
      await tester.pumpAndSettle();
      final escalated = find.byKey(const ValueKey('events-quick-escalated'));
      ...
      expect(queries.last.containsKey('status'), false);
