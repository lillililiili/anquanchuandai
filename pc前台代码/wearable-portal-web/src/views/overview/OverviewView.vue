...
import { phaseLabels } from '@/utils/event-contract'
...
const metrics = [{ key: 'duty', title: '当班人数', note: '名册人员去重，不代表在线人数' }, { key: 'equipment', title: '已领用装备数', note: '明确已领用设备；未知及冲突不计入' }, { key: 'open', title: '未完成事件数', note: '待认领、处理中、待现场核验' }, { key: 'mine', title: '本人待办数', note: '上述未完成事件中分配给我的事件' }]
...
const titles = { ...Object.fromEntries(metrics.map(m => [m.key, m.title])), unknown: '跟进阶段未知事件' }
...
const eventTarget = e => ({ path: '/alarms', query: { siteId: siteId.value, selectedId: e.eventId } })
  ...
  <section class="workbench">
    ...
    <template v-else>
      ...
      <SceneGallery :site-id="siteId" /><div class="workbench-columns">
        <section v-for="block in [{ key: 'open', title: '重点关注', note: '展示未完成事件前 5 项，不推定风险等级。' }, { key: 'mine', title: '我的待办', note: '按事件负责人身份关联，不通过人员佩戴关系推定。' }]" :key="block.key" class="workbench-panel"><header><h2>{{ block.title }}</h2><button class="event-button" :disabled="section(block.key)?.state !== 'AVAILABLE'" @click="open(block.key)">查看全部</button></header><p class="workbench-note">{{ block.note }}</p><DataState v-if="section(block.key)?.state !== 'AVAILABLE'" :state="section(block.key)?.state" :message="section(block.key)?.message" /><p v-else-if="!section(block.key).data.length" class="workbench-empty">当前厂站此范围暂无未完成事件。</p><router-link v-for="e in (section(block.key)?.data || []).slice(0, 5)" :key="e.eventId" class="workbench-event" :to="eventTarget(e)"><div><strong>{{ e.title }}</strong><small>{{ e.deviceCode }} · {{ e.eventId }}</small></div><span>{{ phaseLabels[e.phase] }} →</span></router-link><button v-if="block.key === 'open'" class="unknown-link" :disabled="section('unknown')?.state !== 'AVAILABLE'" @click="open('unknown')">跟进阶段未知：{{ count('unknown') }} · 单独查看，不计入未完成</button></section>
      </div>
      <section v-if="result.data?.collaboration" class="workbench-panel"><header><h2>协同提醒</h2><router-link :to="{ path: '/dispatch', query: { siteId } }">进入调度协同 →</router-link></header><p class="workbench-note">本地会话与事件处置独立；接通或结束不会自动完成事件。</p><DataState v-if="result.data.collaboration.state !== 'AVAILABLE'" :state="result.data.collaboration.state" /><template v-else><p v-if="!result.data.collaboration.data.length && !result.data.broadcasts.data.length">没有进行中的本地会话或待回执广播。</p><p v-for="s in result.data.collaboration.data" :key="s.id">{{ s.title }} · 明确本地会话就绪 {{ s.count }} / {{ s.total }} 个对象</p><p v-for="b in result.data.broadcasts.data" :key="b.id">{{ b.title }} · {{ b.count }} 个对象待回执</p></template></section>
      <section class="workbench-panel"><header><h2>统计与口径</h2><router-link :to="{ path: '/statistics', query: { siteId } }">查看统计分析与导出 →</router-link></header><p class="workbench-note">当前快照与历史区间分开；装备领用率排除未知及冲突，事件完成以显式本地完成记录为准。不提供健康评分，不将设备在线解释为人员健康或视频可用。</p></section>
      <section v-if="result.data?.works" class="workbench-panel"><header><h2>作业监护摘要</h2><router-link :to="{ path: '/supervision', query: { siteId } }">查看全部作业 →</router-link></header><DataState v-if="result.data.works.state !== 'AVAILABLE'" :state="result.data.works.state" :message="result.data.works.message" /><p v-else-if="!result.data.works.data.length">当前厂站没有作业。</p><router-link v-for="w in (result.data.works.data || []).slice(0, 5)" :key="w.workId" class="workbench-event" :to="{ path: '/supervision/' + w.workId, query: { siteId } }"><strong>{{ w.name }}</strong><span>{{ { PENDING: '待开始', ACTIVE: '监护中', PAUSED: '暂停', ENDED: '已结束' }[w.monitorState] }} · {{ w.participantCount }} 人 · {{ w.openEventCount ?? '未知' }} 件未完成事件</span></router-link></section>
      <section class="workbench-panel"><header><h2>快捷入口</h2><span>进入工作区，不启动设备连接</span></header><div class="workbench-shortcuts"><router-link v-for="item in [{ path: '/personnel', title: '查看当班人员' }, { path: '/location', query: { tab: 'live' }, title: '查看位置快照' }, { path: '/video', title: '查看视频设备' }, { path: '/alarms', title: '查看全部事件' }, { path: '/materials', title: '查询现场资料' }]" :key="item.path" :to="workspaceTarget(item, siteId)">{{ item.title }} →</router-link></div></section>
    </template>
    <el-dialog class="v3-metric-dialog" :model-value="!!drill" :title="titles[drill] || '明细'" width="min(900px, 92vw)" @close="drill = ''"><p>当前厂站 · 共 {{ rows.length }} 条 · 与安全总览指标使用同一份数据</p><div class="workbench-drill"><table><thead><tr><th>名称 / 标识</th><th>状态或类型</th><th>查看</th></tr></thead><tbody><tr v-for="item in visibleRows" :key="item.eventId || item.personId || item.deviceId"><td>{{ item.name || item.title }}<small>{{ item.personId || item.deviceCode || item.eventId }}</small></td><td>{{ item.phase ? phaseLabels[item.phase] : item.type ? { HELMET: '安全帽', BELT: '安全带', WATCH: '手表' }[item.type] : '名册当班' }}</td><td><router-link v-if="item.eventId" :to="eventTarget(item)" @click="drill = ''">查看事件 →</router-link><router-link v-else-if="item.personId" :to="{ path: '/personnel/' + item.personId, query: { siteId, returnTo: '/overview?siteId=' + siteId } }" @click="drill = ''">查看人员 →</router-link><router-link v-else :to="{ path: '/equipment/' + item.deviceId, query: { siteId } }" @click="drill = ''">查看装备 →</router-link></td></tr></tbody></table></div><p v-if="!rows.length">当前范围暂无记录。</p><AppPagination :current-page="page" :page-size="20" :total="rows.length" @current-change="page = $event" /></el-dialog>
  </section>
</template>
<style scoped>
