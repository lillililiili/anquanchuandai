...
<template><section class="equipment-page">
  ...
  <WorkspaceState :context="context" :site-id="siteId" :available-site="availableSite" :query="detail" @retry="reload">
    <div v-if="device" class="equipment-detail-grid">
      ...
      <section class="equipment-panel"><h2>设备告警</h2><router-link :to="{ path: '/alarms', query: { siteId, keyword: device.deviceCode, deviceType: device.type } }">查看此设备告警 →</router-link><p class="equipment-note">按设备编号查询；已处理记录可在告警页切换查看。</p></section><section class="equipment-panel"><h2>能力说明</h2><DeviceProfile :device="device" /><p class="equipment-note">本页不会建立媒体连接或发送设备命令。</p></section>
