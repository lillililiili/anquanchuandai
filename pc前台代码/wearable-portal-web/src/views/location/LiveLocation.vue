...
import { positionReason, reliablePosition } from '@/utils/spatial-contract'
...
watch([mapped, () => list.state, () => list.data], () => {
  if (selectedId.value || list.state !== 'READY') return
  if (mapped.value) { select(mapped.value.id); return }
  const first = list.data?.items?.find(i => reliablePosition(i.position))
  if (first) select(first.id)
})
watch(query, q => { keyword.value = q.keyword || '' }, { immediate: true })
const points = computed(() => {
  const item = selected.value
  return item && reliablePosition(item.position) ? [{ ...item.position, recordId: item.id }] : []
})
