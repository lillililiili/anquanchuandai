...
const status = ref(''), teamId = ref('')
const teams = computed(() => (context.data?.teams || []).filter(t => t.siteId === siteId.value))
const teamLabel = f => f.teamId === 'ALL' ? '全部班组' : f.teamName || teams.value.find(t => t.teamId === f.teamId)?.name || '来源未确认'
watch(query, q => { status.value = q.status || ''; teamId.value = q.teamId || '' }, { immediate: true })
...
<template>
  ...
  <div class="s2-fence-grid" :class="{ 'has-selection': !!selectedId }">
    <section class="s2-panel s2-list"><h2>围栏列表</h2><label class="s2-stacked-form">状态<select v-model="status" :disabled="!availableSite" @change="search({ status, ...(enabled ? { teamId } : {}) })"><option value="">全部状态</option><option value="ENABLED">启用</option><option value="DISABLED">停用</option><option value="UNKNOWN">未知</option></select></label>
      <label v-if="enabled" class="s2-stacked-form">适用班组<select v-model="teamId" :disabled="!availableSite" @change="search({ status, teamId })"><option value="">全部围栏</option><option value="ALL">适用于全部班组</option><option v-for="t in teams" :key="t.teamId" :value="t.teamId">{{ t.name }}</option><option value="UNKNOWN">班组未确认</option></select></label><small v-if="enabled" class="s2-note">选择具体班组时，包含适用于全部班组的围栏。</small>
      <WorkspaceState :context="context" :available-site="availableSite" :site-id="siteId" :query="list" @retry="reload"><div class="workspace-list-scroll" tabindex="0" aria-label="列表内容"><button v-for="f in list.data?.items" :key="f.id" class="s2-list-row" :class="{ selected: selectedId === f.id }" :aria-pressed="selectedId === f.id" @click="select(f.id)"><strong>{{ f.name }}</strong><span>{{ spatialLabels[f.status] }}</span><small>班组：{{ teamLabel(f) }}</small><small>版本 {{ f.version || '未知' }}</small></button><p v-if="!list.data?.items.length" class="s2-note">暂无围栏记录</p></div><AppPagination v-if="list.data?.total" :current-page="Number(query.pageNum || 1)" :page-size="Number(query.pageSize || 20)" :total="list.data.total" @current-change="page" /></WorkspaceState>
    </section>
