...
<template><div class="event-scene"><section class="event-section"><h3>告警发生位置</h3><template v-if="detail.location.state === 'AVAILABLE'"><VectorMap :points="[detail.location.data]" :message="positionReason(detail.location.data)" /><p class="event-note">{{ positionReason(detail.location.data) }} · {{ eventTime(detail.location.data.sourceTime) }}</p></template><p v-else class="event-note">{{ detail.location.state === 'FORBIDDEN' ? '无权查看告警位置' : '告警发生位置未知，不使用设备当前位置代替。' }}</p></section><section class="event-section"><h3>现场影像证据</h3><VideoPlayer :device="mediaStub" compact /><DataState v-if="detail.video.state !== 'AVAILABLE'" :state="detail.video.state" :reason="detail.video.reasonCode" compact /><template v-else><router-link v-for="v in detail.video.data" :key="v.id" class="event-link" :to="{ path: '/video/' + v.id, query: { siteId } }">{{ v.name }} · 查看当前设备单路监看 ↗</router-link></template><p class="event-note">当前设备监看不是事件发生时录像；真实拉流未开放。</p></section></div></template>
