...
<template>
  ...
  <div v-else class="related-records">
    <article v-for="(item, index) in section.data" :key="item.workId || item.eventId || index">
      ...
      <p v-else class="muted">{{ formatTime(item.occurredAt) }} · {{ item.status || '状态未知' }}</p>
    </article>
  </div>
</template>
