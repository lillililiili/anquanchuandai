...
const teams = computed(() => (context.data?.teams || []).filter(t => t.siteId === props.siteId))
...
function start() {
  clear(); fieldErrors.value = {}; draft.value = { name: '', nodes: [], appliesTo: '', teamId: '' }; mode.value = 'draw'; open.value = true
}
...
<template>
  ...
  <el-dialog :model-value="open" title="新增围栏" destroy-on-close width="min(950px, 94vw)" append-to-body :close-on-click-modal="false" :before-close="close">
    <form ref="form" novalidate class="local-editor" @submit.prevent="save"><div class="local-fields"><label>围栏名称<input v-model="draft.name" :aria-invalid="!!fieldErrors.name" :aria-describedby="fieldErrors.name ? 'fence-name-error' : undefined" maxlength="100" required :disabled="busy" @input="fieldChanged('name')" /><small v-if="fieldErrors.name" id="fence-name-error" class="fence-field-error">{{ fieldErrors.name }}</small></label><label>适用班组<select aria-label="适用班组" v-model="draft.teamId" :aria-invalid="!!fieldErrors.teamId" :disabled="busy" required @change="fieldChanged('teamId')"><option value="">请选择班组</option><option value="ALL">全部班组</option><option v-for="t in teams" :key="t.teamId" :value="t.teamId">{{ t.name }}</option></select><small v-if="fieldErrors.teamId" class="fence-field-error">{{ fieldErrors.teamId }}</small></label><label>设备类型<select aria-label="设备类型" v-model="draft.appliesTo" :aria-invalid="!!fieldErrors.appliesTo" :aria-describedby="fieldErrors.appliesTo ? 'fence-appliesTo-error' : undefined" required :disabled="busy" @change="fieldChanged('appliesTo')"><option value="">请选择设备类型</option><option value="ALL">全部装备</option><option value="HELMET">安全帽</option><option value="BELT">安全带</option><option value="WATCH">手表</option></select><small v-if="fieldErrors.appliesTo" id="fence-appliesTo-error" class="fence-field-error">{{ fieldErrors.appliesTo }}</small></label></div>
  ...
  <el-dialog v-model="historyOpen" title="围栏版本历史（只读）" width="min(720px, 94vw)" append-to-body><ol class="local-versions"><li v-for="v in versions" :key="v.version"><strong>版本 {{ v.version }} · {{ v.name }}</strong><p>{{ v.action }} · {{ v.status }} · {{ v.recordedAt || '种子快照，操作时间未知' }}</p><p>适用班组：{{ v.teamId === 'ALL' ? '全部班组' : v.teamName || '来源未确认' }}</p><p>{{ v.rule }} · {{ v.ring?.length - 1 }} 个节点</p><details><summary>查看该版本坐标</summary><pre>{{ JSON.stringify(v.ring, null, 2) }}</pre></details></li></ol></el-dialog>
</template>
