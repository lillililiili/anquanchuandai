...
export const openPhases = ['UNHANDLED']
export function buildWorkbench(dataset, role, siteId) {
  ...
  const open = unique(events.filter(e => e.handlingStatus === 'UNHANDLED'), 'eventId')
  const mine = open
  ...
  return { siteId, source: 'MOCK', revision: dataset.meta.businessRevision || 0,
    ...
    unknown: wrap('events', events.filter(e => !['UNHANDLED', 'HANDLED'].includes(e.handlingStatus))) }
}
