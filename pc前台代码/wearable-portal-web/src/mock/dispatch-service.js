...
export function dispatchCommand(d, role, action, q) {
  ...
  if (action === 'group') {
    ...
    d.entities.events.unshift({ eventId, siteId: q.siteId, title: '本地SOS · ' + device.name, sourceSystem: 'FRONTEND_MOCK', sourceEventId: eventId, eventType: category, eventTypeName: '本地紧急求助', phase: 'UNCLAIMED', deviceId: device.deviceId, deviceCode: device.deviceCode, occurredAt: now, receivedAt: now, sourceUpdatedAt: now, freshness: 'FRESH', person: missing('HISTORICAL_ATTRIBUTION_UNKNOWN'), legacyHandled: null, workId: null, ownerUserId: null, version: 1, deviceReport: { category, rawType: 'MOCK_SOS', rawLevel: null, source: 'MOCK_REQUIREMENT', model: device.model, description: '预置面板合成求助，不是设备上报；历史人员归属未知', recovery: 'UNKNOWN', localFeedback: '未发送设备指令', evidence: '纯本地，无厂家协议确认' } })
