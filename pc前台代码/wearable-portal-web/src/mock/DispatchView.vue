...
const relatedSessions = computed(() => sosId.value ? (result.data?.sessions || []).filter(s => s.eventId === sosId.value) : result.data?.sessions || [])
const contactItems = computed(() => {
  const items = result.data?.items || []
  if (!sosId.value) return items
  const targets = (result.data?.suggested || []).filter(c => c.deviceId === sos.value?.deviceId)
  return [...targets, ...items.filter(c => !targets.some(t => t.deviceId === c.deviceId))]
})
const contactFeedback = ref('')
async function focusContacts() {
  if (busy.value) return
  const target = result.data?.suggested.find(c => c.deviceId === sos.value?.deviceId)
  contactFeedback.value = ''
  if (!target) { contactFeedback.value = '求助设备未接入或不可见，无法选择联系对象。'; return }
  if (!allowed.value) contactFeedback.value = '已定位求助设备；当前身份只读，不能发起联系。'
  else {
    const supported = target.modes.includes(mode.value) ? mode.value : ['VOICE', 'VIDEO'].find(m => target.modes.includes(m))
    if (supported) {
      mode.value = supported
      selected.value = [target.deviceId]
      changed()
      contactFeedback.value = '已选中 ' + target.name + '（' + target.deviceCode + '），当前关联人员：' + target.personName + '。请确认后发起呼叫。'
    } else {
      selected.value = []
      contactFeedback.value = '已定位 ' + target.name + '，当前联系能力不可用：' + target.reason
    }
  }
  await nextTick()
  const card = document.getElementById('dispatch-contact-' + target.deviceId)
...
watch(() => [route.fullPath, authorized.value, user.token], () => { clear(); contactFeedback.value = ''; keyword.value = query.value.keyword || ''; selectedContext = ''; reload() }, { immediate: true })
watch(() => result.data, d => {
...
function page(n) { router.push({ path: sosId.value ? '/dispatch/sos/' + sosId.value : '/dispatch', query: { ...query.value, siteId: siteId.value, keyword: keyword.value, pageNum: String(n) } }) }
...
<template><div class="dispatch-page" :class="{ 'is-sos': !!sosId }">
  ...
  <template v-else-if="result.data">
    ...
    <section v-if="sosId" class="dispatch-panel dispatch-sos">
      <template v-if="sos"><h2><span class="sos-urgent-label">紧急 SOS</span> {{ sos.title }}</h2><p>求助事件 {{ sos.eventId }} · {{ alarmLabel(sos) }} · 来源：{{ sos.deviceReport.category }}</p><p>发生 {{ eventTime(sos.occurredAt) }}；接收 {{ eventTime(sos.receivedAt) }}。历史人员归属未知，不用当前领用人回填。</p><p>联系能力：{{ result.data.suggested[0]?.reason || '联系对象未接入' }}</p><p>关联会话：{{ result.data.sessions.filter(s => s.eventId === sos.eventId).map(s => s.id).join('、') || '尚无会话；报警可独立存在' }}</p>
        <p class="sos-context-note">下方用于联系本次求助设备，也可勾选其他设备参与协助。由此发起的会话关联本次 SOS；处理告警与结束通话分别记录。</p><div class="dispatch-actions"><router-link class="dispatch-button" :to="{ path: '/alarms/' + sos.eventId, query: { siteId, returnTo: route.fullPath } }">查看告警与处理记录</router-link><router-link v-if="result.data.suggested.some(c => c.modes.includes('VIDEO'))" class="dispatch-button" :to="{ path: '/video/' + sos.deviceId, query: { siteId, returnTo: route.fullPath } }">查看当前本地视频（非事发录像）</router-link><button class="dispatch-button sos-contact-button" :disabled="busy" @click="focusContacts">联系求助设备 ↓</button></div>
    ...
    <div class="dispatch-columns">
      <div class="dispatch-stack"><section id="dispatch-contacts" tabindex="-1" class="dispatch-panel"><h2>{{ sosId ? '联系求助设备与协助人员' : '联系人与设备' }}</h2><p class="dispatch-note">人员名称仅表达当前明确领用关系；不等于历史求助人。电话仅声明，不提供拨号。</p>
        ...
        <p v-if="contactFeedback" role="status" class="dispatch-contact-feedback">{{ contactFeedback }}</p><div class="dispatch-contacts"><label v-for="c in contactItems" :id="'dispatch-contact-' + c.deviceId" :key="c.deviceId" tabindex="-1" class="dispatch-contact" :class="{ 'is-selected': selected.includes(c.deviceId) }"><input v-model="selected" type="checkbox" :value="c.deviceId" :disabled="!allowed || !c.modes.includes(mode) || busy" @change="changed" /><span><strong><span v-if="sos && c.deviceId === sos.deviceId" class="sos-device-label">本次求助设备 · </span>{{ c.name }} · {{ c.personName }}</strong><small>{{ c.deviceCode }} · 通信 {{ c.communication }}（不代表会话状态）</small><small>{{ c.reason }}</small></span></label></div><p v-if="!contactItems.length">当前筛选没有联系人。</p>
