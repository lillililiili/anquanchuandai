async (page) => {
  ...
  await page.screenshot({ path: 'output/playwright/v5-login.png' });
  ...
  for (const path of ['/overview', '/personnel', '/personnel/9007199254740993101', '/equipment', '/video', '/materials', '/supervision', '/alarms', '/dispatch', '/statistics', '/location?tab=live', '/location?tab=tracks', '/location?tab=fences']) {
    ...
    await page.screenshot({ path: 'output/playwright/v5-' + name + '.png' });
  ...
  await page.screenshot({ path: 'output/playwright/v5-dialog.png' });
  results.push({ path: 'dialog', overflow: await page.evaluate(() => document.documentElement.scrollWidth > innerWidth + 2) });
  ...
  for (const width of [1440, 1024]) {
    ...
    await page.screenshot({ path: 'output/playwright/v5-overview-' + width + '.png' });
    await page.goto(origin + '/#/personnel?siteId=mock-site-1');
    await page.locator('h1').first().waitFor();
    await page.screenshot({ path: 'output/playwright/v5-personnel-' + width + '.png' });
  }
