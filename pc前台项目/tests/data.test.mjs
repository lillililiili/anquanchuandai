...
import * as D from "../src/mock/data.js";
const storage = () => {
  let text = null;
...
test("核验草稿不改变统计，提交后数量、列表、时间线一致且不重复提交", () => {
  ...
  assert.equal(d.stats().unresolved, 6);
...
test("通信流程、成员去重、回执和 SOS 独立于告警", () => {
  ...
  d.sos("join");
  d.sos("end");
  assert.equal(d.stats().events.length, 7);
  assert.equal(d.stats().unresolved, 7);
...
test("旧存档补入新增告警且不覆盖已有事件", () => {
  const mem = storage();
  const old = D.seed();
  old.events = old.events.filter((event) =>
    ["RL-E-0915-001", "RL-E-0915-002", "RL-E-0915-003"].includes(event.id),
  );
  old.events[0].status = "已核验";
  mem.setItem(D.KEY, JSON.stringify(old));
  const d = D.createStore(mem);
  assert.equal(d.event("RL-E-0915-001").status, "已核验");
  assert.equal(d.event("RL-E-0915-004").type, "人员求助");
  assert.equal(d.event("RL-E-0915-005").type, "电子围栏");
  assert.equal(d.event("RL-E-0915-006").type, "安全带挂接");
  assert.equal(d.event("RL-E-0915-007").type, "生命体征");
  assert.equal(d.state.events.length, 7);
});
