...
class _TasksPageState extends State<TasksPage> {
  ...
  Widget _menu({
    ...
    return PopupMenuButton<String>(
      ...
      child: Chip(
        label: Text(label),
        avatar: Icon(Icons.filter_list, size: 17),
      ),
    );
  }
}
...
class _TaskPageState extends State<TaskPage> {
  ...
  Widget build(BuildContext context) {
    final task = _task;
    return QueryPage(
      title: task == null ? '关联作业详情' : textOf(task['title']),
      subtitle: '只读作业背景，工作票与成员状态以记录为准。',
      body: QueryStateView(
        loading: _loading,
        error: _error,
        empty: task == null,
  ...
  String _ticketLabel(JsonMap task) {
    if (task['ticketRequired'] == false) return '无需工作票';
    if (task['ticketRequired'] != true) return '工作票要求未知';
