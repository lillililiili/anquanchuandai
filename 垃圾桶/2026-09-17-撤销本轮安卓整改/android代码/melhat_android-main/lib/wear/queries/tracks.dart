...
class _TracksPageState extends State<TracksPage> {
  ...
  Widget build(BuildContext context) {
    return QueryPage(
      title: '人员轨迹',
      subtitle: '按时间回看位置；轨迹质量与缺失样本需结合现场核对。',
      body: Column(
        children: [
          ...
          if (widget.personId != null && _person != null)
            Padding(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 6),
              child: QueryRow(
                title: textOf(_person!['name']),
                subtitle: textOf(_person!['personCode']),
          ...
          Expanded(
            child: _person == null
                ? const WearEmpty(title: '请先选择人员', detail: '搜索现场人员，再选择需要查看的日期。')
                : QueryStateView(
                    loading: _loading,
                    error: _error,
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    IconButton(
                      tooltip: _playing ? '暂停轨迹' : '播放轨迹',
                      onPressed: _points.length < 2 ? null : _togglePlayback,
                      icon: Icon(
                        _playing
