...
async function initialize() {
  ...
  if (intent.value.deviceId) {
    ...
    if (intent.value.mode === 'return') { const rows = (await read('assignments', { deviceId: d.id })).rows; if (!rows[0]?.personId || rows[0].relation !== 'ASSIGNED') throw new Error('领用领用情况不明、冲突或人员不可见，不能归还'); personId.value = rows[0].personId; fixedReturn.value = true }
  }
