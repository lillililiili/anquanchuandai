<template>
  <div class="app-container events-workspace">
    <ModuleHeader
      module="system"
      title="安全事件"
      description="认领才会占有。已看见不等于已认领。关闭必须满足状态和权限。演示事件带标记。"
    />
    <el-form class="search-form" :inline="true" :model="queryParams">
      ...
      <el-form-item label="状态">
        <el-select v-model="queryParams.status" class="w-140" clearable placeholder="未关闭">
      ...
      <div class="events-detail">
        <template v-if="detail.id">
          ...
          <div class="detail-actions">
            ...
            <el-button v-if="canCloseLow" type="success" @click="openClose">关闭</el-button>
            <el-button v-if="canCloseHigh" type="success" @click="openClose">复核关闭</el-button>
            <el-button v-if="canReview && detail.status === 'closed'" @click="openReopen">重开</el-button>
          ...
          <p v-if="!actions.length" class="detail-line">暂无动作。已看见 / 已认领 / 已关闭会分别出现在这里。</p>
        </template>
    ...
    <el-dialog v-model="closeOpen" title="关闭事件" width="420px">
      <el-form label-width="80px">
...
const canReview = computed(() => userStore.canReviewEvent)
const canSimulate = computed(() => import.meta.env.MODE !== 'production' && (canClaim.value || userStore.isPlatformAdmin))
const canCloseLow = computed(() => canClaim.value && detail.value.status === 'handling' && detail.value.severity === 'low')
const canCloseHigh = computed(() => canReview.value && detail.value.status === 'pending_review' && detail.value.severity === 'high')
...
const listHead = computed(() => {
  if (queryParams.status === 'closed') return '已关闭'
...
function actionLabel(action) {
  const map = { ack: '已看见', claim: '已认领', handle: '处置', transfer: '转交', close: '已关闭', reopen: '重开', escalate: '升级', review: '复核' }
