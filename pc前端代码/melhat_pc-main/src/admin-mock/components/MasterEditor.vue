<template>
  <form class="master-form" novalidate @submit.prevent="submit">
    ...
    <div class="form-grid">
      <label v-if="!['accounts', 'roles', 'dutyShifts'].includes(entity)">编号<input id="field-code" v-model="form.code" maxlength="50" :aria-invalid="!!errors.code" aria-describedby="error-code" /><small id="error-code" class="error">{{ errors.code }}</small></label>
      <label v-if="entity === 'accounts'">登录名<input id="field-loginName" v-model="form.loginName" maxlength="50" :disabled="!!record" :aria-invalid="!!errors.loginName" aria-describedby="error-loginName" /><small id="error-loginName" class="error">{{ errors.loginName }}</small></label>
      <label>名称 / 姓名<input id="field-name" v-model="form.name" aria-label="名称 / 姓名" maxlength="100" :aria-invalid="!!errors.name" aria-describedby="error-name" /><small id="error-name" class="error">{{ errors.name }}</small></label>
    ...
    <template v-if="entity === 'roles'">
      <fieldset id="field-operations"><legend>允许操作</legend><div class="choice-grid"><label v-for="op in OPERATIONS" :key="op"><input v-model="form.operations" type="checkbox" :value="op" />{{ operationNames[op] || op }}</label></div><small class="error">{{ errors.operations }}</small></fieldset>
