<template>
  <section>
    <div class="page-heading"><div><span class="eyebrow">管理中心 / {{ route.meta.group === 'people' ? '人员组织' : '权限协作' }}</span><h1>{{ route.meta.title }}</h1><p class="muted">{{ guidance[entity] }}</p></div><span class="badge">A1 · 本页内存办理</span></div>
    <nav class="workspace-tabs" aria-label="工作区导航"><router-link v-for="item in WORKSPACES.filter(x => x.group === route.meta.group)" :key="item.path" :class="{ active: item.path === route.path }" :to="{ path: item.path, query: { siteId: store.siteId } }">{{ item.title }}</router-link></nav>
    ...
    <ModalPanel heading-id="reset-credential-heading" :open="!!resetAccount" title="重置本地凭据" @close="closeReset">
      <form v-if="resetAccount" @submit.prevent="resetCredential"><p>账号：{{ resetAccount.name }}</p><p class="notice">未修改真实密码，重新选择预置身份可进入；不代表其他浏览器或真实终端已退出。</p><label>重置原因<textarea v-model="resetReason" required maxlength="500" /></label><label class="inline-check"><input v-model="resetConfirmed" type="checkbox" required />确认仅递增本地凭据版本，不改变人员、装备或维修关系</label><p v-if="saveError" ref="resetErrorBox" tabindex="-1" class="notice error" role="alert">{{ saveError.message }}</p><button class="button primary" :disabled="busy || !resetConfirmed">确认重置本地凭据</button></form>
    </ModalPanel>
...
async function save(data) {
  ...
  try {
    ...
    busy.value = false; dirty.value = false; editorOpen.value = false; notice.value = `已保存${ENTITIES[entity.value].title}“${result.data.name}”（仅本页内存）。`; await load()
...
async function confirmAction() {
  busy.value = true; saveError.value = null; saveController = new AbortController()
  try { const { row, remove } = action.value; const type = `${entity.value}.${remove ? 'delete' : 'status'}`, command = { siteId: store.siteId, id: row.id, expectedVersion: row.version, enabled: !row.enabled, operationId }; if (entity.value === 'roles' && await preparePreview(type, command)) return; await provider.execute(type, command, { signal: saveController.signal }); busy.value = false; action.value = null; notice.value = '操作已完成（仅本页内存），相关记录未级联修改。'; await load() }
...
async function resetCredential() {
  busy.value = true; saveError.value = null; saveController = new AbortController()
  try { await provider.execute('accounts.resetCredential', { siteId: store.siteId, id: resetAccount.value.id, expectedVersion: resetAccount.value.version, operationId, reason: resetReason.value, confirm: resetConfirmed.value }, { signal: saveController.signal }); resetAccount.value = null; resetReason.value = ''; notice.value = '本地凭据版本已更新，未修改真实密码。'; await load() }
  catch (e) { if (e.name !== 'AbortError') saveError.value = e }
  finally { busy.value = false }
}
