  ...
  <section>
    ...
    <template v-if="!detail">
      <section class="panel master-list">
        <form class="list-filters" @submit.prevent="search"><label>编号 / 名称<input v-model="keyword" maxlength="100" /></label><label>状态<select v-model="status"><option value="">全部</option><option value="enabled">启用</option><option value="disabled">停用</option></select></label><div class="filter-actions"><button class="button primary">查询</button><button class="button" type="button" @click="keyword = ''; status = ''; search()">重置</button></div></form><div class="section-heading"><h2>授权范围内的完整协助组</h2><div><button class="button primary" :disabled="!writable || loading || error || data?.availability !== 'AVAILABLE'" @click="edit(null)">新建协助组</button><small v-if="!writable" class="disabled-reason">无协助组维护权限</small></div></div>
        <QueryState :data="data" :loading="loading" :error="error" @retry="load"><div v-if="data?.rows?.length" class="table-scroll"><table v-table class="admin-table "><thead><tr><th data-width="160">协助组编号</th><th data-width="200">协助组名称</th><th data-width="100" class="number-cell">人员数</th><th data-width="100" class="status-cell">状态</th><th data-width="240">通知策略</th><th data-width="200" class="table-actions">操作</th></tr></thead><tbody><tr v-for="g in data.rows" :key="g.id"><td>{{ g.code }}</td><td>{{ g.name }}</td><td class="number-cell">{{ g.memberCount }}</td><td class="status-cell"><span class="badge" :class="{ warning: !g.enabled }">{{ g.enabled ? '启用' : '停用' }}</span></td><td>{{ !g.sos ? '策略待配置' : g.enabled && g.sos.enabled ? '本地配置已启用，未通知' : '通知不生效' }}</td><td class="table-actions"><div class="row-actions"><button class="button" @click="inspect(g)">查看档案</button><button class="button" :disabled="!g.writable" :title="!g.writable ? '无完整范围维护权限' : undefined" @click="edit(g)">编辑</button></div></td></tr></tbody></table></div><p v-else class="query-state">无匹配协助组，不代表所有厂站均无协助组。</p><div class="pagination"><span>共 {{ data?.total || 0 }} 条 · 第 {{ pageNum }} 页</span><label>每页<select :value="pageSize" @change="navigate({ pageSize: $event.target.value, pageNum: '1' })"><option>20</option><option>50</option><option>100</option></select></label><button class="button" :disabled="pageNum <= 1" @click="navigate({ pageNum: String(pageNum - 1) })">上一页</button><button class="button" :disabled="pageNum * pageSize >= data?.total" @click="navigate({ pageNum: String(pageNum + 1) })">下一页</button></div></QueryState>
      </section>
    </template>
    <template v-else>
      <router-link class="button" :to="back">返回协助组列表</router-link>
      <QueryState :data="data" :error="error" :loading="loading" @retry="load"><template v-if="data?.id"><section class="panel master-list"><div class="section-heading"><h2>{{ data.name }}</h2><div class="actions"><button class="button" :disabled="!data.writable" @click="edit(data)">编辑协助组</button><button class="button" :disabled="!data.writable || busy" @click="prepareStatus">{{ data.enabled ? '停用协助组' : '启用协助组' }}</button></div></div><p v-if="!data.writable" class="disabled-reason">无完整范围维护权限</p><dl class="record-fields"><dt>编号</dt><dd>{{ data.code }}</dd><dt>版本</dt><dd>{{ data.version }}</dd><dt>管理区域</dt><dd>{{ data.areaName }}</dd><dt>负责人</dt><dd>{{ memberName(data.leaderId) }}</dd><dt>成员数</dt><dd>{{ data.members.length }} 人（不按终端数统计）</dd><dt>SOS 策略</dt><dd>{{ !data.sos ? '待配置' : data.notificationEffective ? '本地策略启用' : '不生效' }}</dd><dt>接收成员</dt><dd>{{ data.sos?.recipientIds.map(memberName).join('、') || '未选择 / 待配置' }}</dd><dt>备注</dt><dd>{{ data.remark || '未填写' }}</dd></dl><p class="notice">仅保存本地通知策略，未发送 SOS 或其他通知；成员、负责人、接收资格均不授予账号或设备权限。</p></section>
        ...
        <section class="panel master-list"><h2>配置历史 · 保留每次修改记录</h2><QueryState :data="history" :error="historyError" :loading="historyLoading" @retry="loadHistory"><article v-for="h in history?.rows || []" :key="h.id" class="binding-card"><h3>v{{ h.version }} · {{ h.name }} · {{ h.enabled ? '启用' : '停用' }}</h3><p>{{ utcTime(h.occurredAt) }} UTC · {{ h.operatorName }} · {{ h.action }}</p><p>成员：{{ h.members.map(p => p.name).join('、') }}</p><p>负责人：{{ h.leader?.name || '待配置' }}；接收人：{{ h.sos?.recipients.map(p => p.name).join('、') || '待配置 / 未选择' }}</p><small>{{ h.source }}</small></article><p v-if="!history?.rows?.length" class="muted">没有配置历史；初始数据不计入操作记录。</p><div class="pagination"><span>共 {{ history?.total || 0 }} 条</span><button class="button" :disabled="historyPage <= 1" @click="historyPage--; loadHistory()">上一页配置</button><button class="button" :disabled="historyPage * 20 >= history?.total" @click="historyPage++; loadHistory()">下一页配置</button></div></QueryState></section>
      </template></QueryState>
    </template>
...
const relationNames = { UNASSIGNED: '未领用', ASSIGNED: '已领用', UNKNOWN: '未知', CONFLICT: '领用记录有矛盾', FORBIDDEN: '无权查看', UNAVAILABLE: '无权查看装备' }, communicationNames = { ONLINE: '通信在线（不代表可呼叫）', OFFLINE: '离线', UNKNOWN: '连接状态不明', NOT_CONNECTED: '通信未接入' }
