<template>
  <div class="admin-shell" :class="{ compact }" :data-section="visualSection">
    ...
    <header class="topbar">
      ...
      <span class="muted" title="业务服务尚未接入；使用本地预置数据，刷新将恢复初始状态">服务未接入</span>
    ...
    <aside class="sidebar">
      <div class="nav-heading"><span v-if="!compact">管理工作区</span><button :aria-expanded="!compact" aria-label="折叠菜单" class="button text-button" @click="compact = !compact">{{ compact ? '展开' : '收起' }}</button></div>
    ...
    <main id="main-content" class="main-content" tabindex="-1">
      <p v-if="contextError" class="notice error" role="alert">{{ contextError }} <button class="button" @click="initialize">重试上下文</button></p>
      ...
      <footer class="workspace-footer">业务服务未接入 · 数据保存在当前页面内存 · 不代表设备已接入或真实业务验收通过</footer>
    </main>
    <AssignmentPanel />
    <MaintenancePanel />
    <ModalPanel heading-id="scene-heading" :open="panel" title="预置场景控制" @close="panel = false">
      <p class="notice">确定性场景，只改变本后台查询表现，不修改种子事实，不访问外部系统。</p>
      <form class="scenario-form" @submit.prevent="applyScenario">
        ...
        <label class="check"><input v-model="slow" type="checkbox" />下一次查询延迟3秒（验证切站竞态）</label>
      ...
      <hr><h3>会话与数据</h3><p class="muted">当前会话失效会返回登录页；重置还会恢复全部初始预置数据。</p>
      <div class="actions"><button class="button" @click="provider.invalidate()">使当前会话失效</button><button class="button danger" @click="confirmReset = true">恢复初始预置数据</button></div>
      <section v-if="confirmReset" class="reset-confirm" role="alert"><h3>确认重置本后台预置？</h3><p>本页内存数据、场景和本地身份将清除，随后返回登录。前台、原PC及数据库不受影响。</p><div class="actions"><button class="button danger" @click="provider.reset()">确认重置并退出</button><button class="button" @click="confirmReset = false">保留数据</button></div></section>
    </ModalPanel>
  </div>
</template>
<script setup>
...
const maintenanceQueryNames = { maintenanceOrders: '维修工单列表', maintenanceOrder: '维修工单详情', maintenanceRecords: '维修过程记录', deviceLifecycleHistory: '生命周期记录' }
...
const unsubscribe = provider.subscribe(event => {
  if (['identity', 'expired', 'reset'].includes(event.kind)) {
    ...
    store.notice = event.kind === 'expired' ? '登录已失效，请重新登录。' : event.kind === 'reset' ? '本后台预置数据已重置。' : ''
