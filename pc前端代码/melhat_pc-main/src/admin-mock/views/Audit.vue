<template>
  <section>
    <div class="page-heading"><div><span class="eyebrow">管理中心 / 追溯记录</span><h1>审计记录</h1><p class="muted">当前厂站：{{ siteName }} · 前端内存模拟记录，刷新后不保留。</p></div><button class="button" :disabled="exporting || list.loading.value" @click="download">{{ exporting ? '正在生成…' : '导出全部筛选结果' }}</button></div>
    ...
    <section class="panel master-list">
      <form class="list-filters" @submit.prevent="search">
        ...
        <label>结果<select v-model="filters.result"><option value="">全部</option><option v-for="(label, value) in AUDIT_RESULTS" :key="value" :value="value">{{ label }}</option></select></label><label>开始日期（UTC）<input v-model="filters.dateFrom" type="date" /></label><label>结束日期（UTC）<input v-model="filters.dateTo" type="date" /></label><button class="button primary">查询</button><button class="button" type="button" @click="reset">清除筛选</button>
      </form>
      <QueryState :data="list.data.value" :loading="list.loading.value" :error="list.error.value" @retry="load">
        <template v-if="list.data.value?.rows"><div class="device-table-wrap"><table class="device-table"><caption class="sr-only">当前厂站授权范围内的审计记录</caption><thead><tr><th>时间（UTC）</th><th>操作账号</th><th>业务对象</th><th>动作</th><th>结果</th><th>操作</th></tr></thead><tbody><tr v-for="row in list.data.value.rows" :key="row.id"><td>{{ row.occurredAt }}<small>{{ row.id }}</small></td><td>{{ row.actorName }}</td><td>{{ row.objectId }}</td><td>{{ row.action }}</td><td>{{ row.result }}</td><td><button class="button" @click="openDetail(row.id)">查看详情</button></td></tr></tbody></table></div><p v-if="!list.data.value.rows.length" class="query-state">当前筛选下暂无审计记录。</p><div class="pagination"><span>共 {{ list.data.value.total }} 条 · 第 {{ pageNum }} 页</span><label>每页<select v-model.number="pageSize" @change="search"><option :value="20">20</option><option :value="50">50</option><option :value="100">100</option></select></label><button class="button" :disabled="pageNum <= 1" @click="turn(-1)">上一页</button><button class="button" :disabled="pageNum * pageSize >= list.data.value.total" @click="turn(1)">下一页</button></div></template>
      </QueryState>
    </section>
    <ModalPanel :open="!!selectedId" side title="审计详情" heading-id="audit-detail-heading" @close="closeDetail">
      <QueryState :data="detail.data.value" :loading="detail.loading.value" :error="detail.error.value" @retry="loadDetail"><template v-if="detail.data.value?.id"><dl class="record-fields"><template v-for="(label, key) in detailFields" :key="key"><dt>{{ label }}</dt><dd>{{ detail.data.value[key] || '未记录' }}</dd></template></dl><p class="notice">以下内容为操作发生时的历史记录（敏感信息已隐藏）；对象删除或改名不会覆盖历史名称。</p><h3>修改内容</h3><div class="device-table-wrap"><table class="device-table"><thead><tr><th>字段</th><th>变更前</th><th>变更后</th></tr></thead><tbody><tr v-for="item in differences" :key="item.key"><td>{{ item.key }}</td><td><pre>{{ json(item.before) }}</pre></td><td><pre>{{ json(item.after) }}</pre></td></tr></tbody></table></div><p v-if="!differences.length">没有可展示的修改内容。</p><h3>修改前记录</h3><pre class="audit-snapshot">{{ json(detail.data.value.before) }}</pre><h3>修改后记录</h3><pre class="audit-snapshot">{{ json(detail.data.value.after) }}</pre></template></QueryState>
    </ModalPanel>
  </section>
</template>
<script setup>
...
async function download() {
  ...
  try {
    ...
    if (typeof result.data?.csv !== 'string') throw new Error('当前审计来源未接入，无法导出')
    const url = URL.createObjectURL(new Blob([result.data.csv], { type: 'text/csv;charset=utf-8' })), link = document.createElement('a')
    try { link.href = url; link.download = result.data.filename; document.body.appendChild(link); link.click(); exportNotice.value = `已生成 ${result.data.total} 条模拟审计记录。` } finally { link.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000) }
...
<style scoped>
.audit-snapshot, pre { white-space: pre-wrap; overflow-wrap: anywhere; max-width: 100%; font-size: 12px; }
.audit-snapshot { background: var(--canvas, #f5f7fa); border-radius: 8px; padding: 16px; }
