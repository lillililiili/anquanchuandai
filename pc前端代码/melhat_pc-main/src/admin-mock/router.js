...
export const router = createRouter({ history: createWebHashHistory(), routes: [
  ...
  { path: '/admin', component: Layout, children: [
    ...MENU.filter(m => m.path !== '/admin/assets/devices' && !WORKSPACES.some(w => w.path === m.path)).map(m => ({ path: m.path, component: m.path === '/admin/overview' ? Overview : Placeholder, meta: m })),
