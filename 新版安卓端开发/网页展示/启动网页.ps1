param([int]$Port = 18766, [switch]$NoBrowser, [switch]$Stop)
...
try {
  ...
  if ($running.Content -eq 'rolling-android-preview') {
  ...
  while ($listener.IsListening) {
    ...
    try {
      ...
      if ($requestPath -eq '/__preview_health') {
        $bytes = [Text.Encoding]::UTF8.GetBytes('rolling-android-preview')
